export const pascalTriangle = lineNumber => {
  const currentLine = [1];
  for (let numIndex = 1, currentLineSize = lineNumber + 1; numIndex < currentLineSize; numIndex += 1) {
    currentLine[numIndex] = currentLine[numIndex - 1] * (lineNumber - numIndex + 1) / numIndex;
  }
  return currentLine;
}

export const pascalTriangleRecursive = lineNumber => {
  if (lineNumber === 0) {
    return [1];
  }

  const previousLineSize = lineNumber;
  const currentLineSize = lineNumber + 1;

  const currentLine = [];
  const previousLine = pascalTriangleRecursive(lineNumber - 1);

  for (let numIndex = 0; numIndex < currentLineSize; numIndex += 1) {
    const leftCoefficient = (numIndex - 1) >= 0 ? previousLine[numIndex - 1] : 0;
    const rightCoefficient = numIndex < previousLineSize ? previousLine[numIndex] : 0;
    currentLine[numIndex] = leftCoefficient + rightCoefficient;
  }

  return currentLine;
}